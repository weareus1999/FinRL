...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.cash_scaling = cash_scaling
        self.day = day
        self.df = df
        self.stock_dim = stock_dim
        self.hmax = hmax
        self.num_stock_shares = num_stock_shares
        self.initial_amount = initial_amount * self.cash_scaling  # get the initial cash
        ...
        self.asset_memory = [
            self.initial_amount/self.cash_scaling,
            + np.sum(
        ...
        def _do_sell_normal():
            if (
                ...
                if self.state[index + self.stock_dim + 1] > 0:
                    ...
                    # update balance
                    self.state[0] = self.state[0] + sell_amount
    ...
    def _buy_stock(self, index, action):
        def _do_buy():
            if (
                ...
                available_amount = (self.state[0]/self.cash_scaling)// (
    ...
    def step(self, actions):
        ...
        info = {}  # your existing info (or new dictionary)
        info["current_obs"] = self.state  # or whichever observation you want to record
        info["date"] = pd.to_datetime(self._get_date()).strftime("%Y-%m-%d")
    ...
    def _initiate_state(self):
        if self.initial:
            ...
            if len(self.df.tic.unique()) > 1:
                # For multiple stocks:
                # Get close prices as a list (if already a Series, use .values.tolist(); otherwise wrap it)
                if hasattr(self.data.close, "values"):
                    close_prices = self.data.close.values.tolist()
                else:
                    close_prices = [self.data.close]
                # Process each technical indicator similarly
                tech_data = []
                for tech in self.tech_indicator_list:
                    if hasattr(self.data[tech], "values"):
                        tech_data.extend(self.data[tech].values.tolist())
                    else:
                        tech_data.append(self.data[tech])
...
from scipy.stats import multivariate_normal  # if needed for later calculations


class StockTradingEnvPrescaled(gym.Env):
    """
    Stock Trading Environment with Fixed Start (day 0), Cash Prescaling, and Reward Scaling.

    Internally, all trading calculations use the actual (unscaled) cash (e.g. 1e6).
    However, the observation returned to the RL agent reports cash as:

        scaled_cash = actual_cash / cash_scaling

    Moreover, the reward (change in total asset) is divided by cash_scaling so that the target
    values are on a similar scale. This environment always starts at day 0.

    In multi-stock mode, the state is assumed to be composed as follows:
      state = [scaled_cash] + close_prices + current_holdings + tech_indicator_values
    where close_prices and current_holdings are lists of length N (number of stocks), and
    tech_indicator_values is the concatenation of the values for each indicator for each stock.

    Thus the expected state dimension is:
      state_space = 1 + 2*N + (len(tech_indicator_list) * N)
    """
    metadata = {"render.modes": ["human"]}
    ...
    def __init__(
            ...
            returns_factor: float = 0.5,
            max_episode_length: int = 90,  # number of days per episode
            cash_scaling: float = 1000.0  # RL sees cash = actual_cash / cash_scaling
    ):
        super().__init__()
        self.cash_scaling = cash_scaling

        # Store the dataframe (assumed to be preprocessed so that in multi-stock mode,
        # each date appears multiple times, one row per stock)
        self.df = df.copy()
        self.stock_dim = stock_dim
        self.hmax = hmax
        self.num_stock_shares = num_stock_shares

        # Save the original (unscaled) cash for trading calculations.
        self.actual_cash = initial_amount
        self.original_cash = initial_amount
        self.buy_cost_pct = buy_cost_pct
        self.sell_cost_pct = sell_cost_pct
        self.reward_scaling = reward_scaling
        self.state_space = state_space  # Expected length of state vector.
        self.action_space_dim = action_space
        self.tech_indicator_list = tech_indicator_list
        ...
        # Fixed start at day 0.
        self.day = 0
        # In multi-stock mode, we expect that all rows corresponding to day 0 have the same date.
        # Here we use .loc with the index 0 if multi-stock; if single-stock, self.data is a Series.
        # if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
        #     self.data = self.df[self.df["date"] == self.df.iloc[0]["date"]]
        # else:
        #     self.data = self.df.iloc[0]
        #print("beforedf", df)
        self.data = self.df.loc[self.day, :]
        #print("afterdf", self.data, self.day)
        self.unique_days = np.array(sorted(self.df["date"].unique()))
        print("UNQEDAYS", len(self.unique_days))
        self.max_episode_length = max_episode_length  # Could be set to len(df) for full episode
        ...
        # Initialize the state (observation); note that cash is scaled.
        print("calling from init")
        self.state = self._initiate_state()

        # Compute the initial total asset using unscaled cash and current holdings.
        if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
            # In multi-stock mode, assume self.data is a DataFrame.
            holdings_value = np.sum(np.array(self.num_stock_shares) * np.array(self.data["close"].values.tolist()))
        else:
            holdings_value = np.sum(np.array(self.num_stock_shares) * np.array([self.data.close]))
        self.asset_memory = [self.actual_cash + holdings_value]
        self.rewards_memory = []
        self.actions_memory = []
        self.state_memory = []
        self.date_memory = [self._get_date()]
    ...
    def _initiate_state(self):
        ...
        The first element is the scaled cash: actual_cash / cash_scaling.
        Then follow the close prices, current stock holdings, and technical indicator values.
        In multi-stock mode, self.data is assumed to be a DataFrame containing all stocks for the current date.
        """
        '''
        scaled_cash = self.actual_cash / self.cash_scaling
        if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
            # Multiple stocks.
            close_prices = self.data["close"].values.tolist() if hasattr(self.data["close"], "values") else [
                self.data["close"]]
            tech_data = []
            for tech in self.tech_indicator_list:
                tech_vals = self.data[tech].values.tolist() if hasattr(self.data[tech], "values") else [self.data[tech]]
                tech_data.extend(tech_vals)
            state = [scaled_cash] + close_prices + self.num_stock_shares + tech_data
        else:
            # Single stock.
            state = ([scaled_cash] +
                     [self.data.close] +
        ...
        return state
        '''
        print("In intitial state", self.data)
        if self.initial:
            if len(self.df.tic.unique()) > 1:
                # Multiple stocks:
                close_prices = self.data.close.values.tolist() if hasattr(self.data.close, "values") else [
                ...
                for tech in self.tech_indicator_list:
                    if hasattr(self.data[tech], "values"):
                        ...
                        tech_data.append(self.data[tech])
                state = ([self.actual_cash / self.cash_scaling] + close_prices + self.num_stock_shares + tech_data)
            else:
                # Single stock:
                state = ([self.actual_cash / self.cash_scaling] +
                         [self.data.close] +
                         [0] * self.stock_dim +
                         sum(([self.data[tech]] for tech in self.tech_indicator_list), []))
        else:
            if len(self.df.tic.unique()) > 1:
                ...
                for tech in self.tech_indicator_list:
                    if hasattr(self.data[tech], "values"):
                        ...
                        tech_data.append(self.data[tech])
                state = ([self.previous_state[0]] + close_prices +
                         self.previous_state[(self.stock_dim + 1):(self.stock_dim * 2 + 1)] + tech_data)
            else:
                state = ([self.previous_state[0]] +
                         [self.data.close] +
                         self.previous_state[(self.stock_dim + 1):(self.stock_dim * 2 + 1)] +
                         sum(([self.data[tech]] for tech in self.tech_indicator_list), []))
        print("lenofstate", len(state))
        return state

    def _update_state(self):
        """
        Update the state vector for the new day.
        The observation returns the scaled cash as the first element.
        """
        scaled_cash = self.actual_cash / self.cash_scaling
        if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
            state = ([scaled_cash] +
    ...
    def _get_date(self):
        """Return the current date from the data row.
           In multi-stock mode, self.data is a DataFrame and we take the unique date from its 'date' column.
        """
        """
                if len(self.df.tic.unique()) > 1:
            date_val = self.data.date
    ...
    def _sell_stock(self, index, action):
        """Execute a sell order and update actual cash."""
    ...
    def _buy_stock(self, index, action):
        """Execute a buy order and update actual cash."""

        def _do_buy():
            if self.state[index + 2 * self.stock_dim + 1] != True:
                available_amount = self.actual_cash // (self.state[index + 1] * (1 + self.buy_cost_pct[index]))
                buy_num_shares = min(available_amount, action)
                buy_amount = self.state[index + 1] * buy_num_shares * (1 + self.buy_cost_pct[index])
                self.actual_cash -= buy_amount
                self.state[index + self.stock_dim + 1] += buy_num_shares
    ...
    def step(self, actions):
        """
        Take an action for one day, update state and compute scaled reward.
        Returns:
        ...
        # Fixed start: we use the day index directly.
        if self.day >= self.max_episode_length - 1 or self.day >= len(self.df)//self.stock_dim- 1:
            return self.state, 0.0, True, False, {}
        ...
        # Compute initial total asset (using actual cash).
        begin_total_asset = self.actual_cash + np.sum(
            np.array(self.state[1:1 + self.stock_dim]) *
            np.array(self.state[self.stock_dim + 1:self.stock_dim * 2 + 1])
        )
        ...
        self.day += 1
        if self.day >= len(self.df)//self.stock_dim:
            return self.state, 0.0, True, False, {}

        #self.data = self.df.iloc[self.day]
        self.data = self.df.loc[self.day, :]
        # if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
        #     self.data = self.df[self.df["date"] == self.df.iloc[self.day]["date"]]
        # else:
        #     self.data = self.df.iloc[0]
        self.state = self._update_state()

        # Compute new total asset.
        end_total_asset = self.actual_cash + np.sum(
            np.array(self.state[1:1 + self.stock_dim]) *
            np.array(self.state[self.stock_dim + 1:self.stock_dim * 2 + 1])
        )
        trade_profit = end_total_asset - begin_total_asset
        ...
        # Scale the reward so that it is consistent with scaled cash.
        reward = (portfolio_reward / self.cash_scaling) * self.reward_scaling
        reward = np.clip(reward, -10, 10)
        self.reward = reward
        ...
        info = {"current_obs": self.state,
                "date": pd.to_datetime(self._get_date()).strftime("%Y-%m-%d")}
        return self.state, self.reward, False, False, info

    def reset(self, *, seed=None, options=None):
        """
        Reset the environment for a new episode.
        Fixed start: always start at day 0.
        """
        ...
        # Reset actual cash to the original unscaled value.
        self.actual_cash = self.original_cash
        print("Calling from reset", self.data)
        self.state = self._initiate_state()
        self.reward = 0
        self.turbulence = 0
        self.cost = 0
        ...
        # else:
        #     holdings_value = np.sum(np.array(self.num_stock_shares) * np.array([self.data.close]))
        self.asset_memory =  [self.actual_cash +
        ...
        """
                Reset the environment for a new episode.
                Randomly select a start day such that there are enough days for the episode.
                """
        max_start = len(self.unique_days) - self.max_episode_length
        self.start_day = np.random.randint(0, max_start)
        self.end_day = self.start_day + self.max_episode_length
    ...
    def save_state_memory(self):
        if "tic" in self.df.columns and len(self.df.tic.unique()) > 1:
            ...
            df_states = pd.DataFrame(self.state_memory)
            df_states.index = df_date["date"]
        else:
            df_states = pd.DataFrame({"date": self.date_memory[:-1], "states": self.state_memory})
        return df_states
...
# (If needed, import your indicator utilities such as BollingerBands, etc.)




class StockTradingEnvRandomStartsPrescaled(gym.Env):
    """
    Stock Trading Environment with Random Starts and Prescaled Cash.

    Internally, trading is done using the actual (unscaled) cash (e.g. 1e6).
    However, the observation provided to the RL agent reports cash as:

        scaled_cash = actual_cash / cash_scaling

    Moreover, the reward signal (i.e. change in total asset) is also scaled by dividing
    by cash_scaling so that both the observation and target values are on similar scales.
    """
    metadata = {"render.modes": ["human"]}
    ...
    def __init__(
            ...
            returns_factor: float = 0.5,
            max_episode_length: int = 90,  # number of days per episode
            cash_scaling: float = 1000.0  # RL sees cash as (actual_cash / cash_scaling)
    ):
        super().__init__()
        self.cash_scaling = cash_scaling

        # Save dataframe and trading parameters.
        self.df = df.copy()
        self.stock_dim = stock_dim
        self.hmax = hmax
        self.num_stock_shares = num_stock_shares

        # Save the original (unscaled) cash and initialize actual cash.
        self.original_cash = initial_amount
        self.actual_cash = initial_amount
        ...
        # Set initial day and get the first data row.
        self.day = day
        self.data = self.df.iloc[self.day]
        self.unique_days = np.array(sorted(self.df["date"].unique()))
        print("UNQEDAYS", len(self.unique_days))
        ...
        # Episode variables; these are set in reset().
        self.start_day = 0
        self.end_day = 0
        self.terminal = False
        self.episode = 0

        # Initialize state (observation). Note that the first element (cash) is reported as scaled.
        self.state = self._initiate_state()

        # Compute the initial total asset using the actual (unscaled) cash.
        self.asset_memory = [self.actual_cash +
                             np.sum(np.array(self.num_stock_shares) * np.array(self.state[1:1 + self.stock_dim]))]
        self.rewards_memory = []
        self.actions_memory = []
    ...
    def _initiate_state(self):
        ...
        The first element is the scaled cash (actual_cash / cash_scaling).
        The remaining elements include close prices, current stock holdings, and technical indicators.
        """
        if self.initial:
            if len(self.df.tic.unique()) > 1:
                # Multiple stocks:
    ...
    def _update_state(self):
        """
        Update the state vector for the new day.
        The first element (cash) is reported as scaled.
        """
        if len(self.df.tic.unique()) > 1:
            state = ([self.actual_cash / self.cash_scaling] +
                     self.data.close.values.tolist() +
    ...
    def step(self, actions):
        ...
        # Compute the initial total asset (using actual cash).
        begin_total_asset = self.actual_cash + sum(
            np.array(self.state[1:1 + self.stock_dim]) *
            np.array(self.state[self.stock_dim + 1:self.stock_dim * 2 + 1])
        )
        ...
        # Compute the new total asset.
        end_total_asset = self.actual_cash + sum(
            np.array(self.state[1:1 + self.stock_dim]) *
            np.array(self.state[self.stock_dim + 1:self.stock_dim * 2 + 1])
        )
        trade_profit = end_total_asset - begin_total_asset
...
class PricePredictionEnvNoRandomOld(gym.Env):
    """
...
class PricePredictionEnvNoRandom(gym.Env):
    """
    A simplified environment for price prediction without random starts.
    This environment is intended for inference/prediction. It always starts at a fixed
    row (start_idx) and proceeds chronologically until the end of the episode.

    Observations: A vector of technical indicators for the current day.
    Action: A single float (normalized in [-1,1]) representing the predicted price.
    Reward: Based on the prediction error (using an exponential or linear function) and
            an extra penalty for a large change between consecutive predictions.
    """

    def __init__(self, df: pd.DataFrame, tech_indicator_list: list[str],
                 reward_type: str = "exp", reward_scale: float = 1.0,
                 start_idx: int = 0, max_episode_length: int = None,
                 penalty_factor: float = 0.0001, deviation_factor: float = 0.01,):
        """
        Args:
            df: DataFrame with at least 'date' and 'close' columns plus technical indicator columns.
            tech_indicator_list: List of column names used as features.
            reward_type: "exp" for exponential reward or "linear" for negative absolute error.
            reward_scale: Scale factor used in the reward function.
            start_idx: Row index in df at which to start the episode.
            max_episode_length: Maximum number of steps in an episode (if None, run until the end of df).
            penalty_factor: Factor that scales the penalty applied for large changes between consecutive predictions.
        """
        super().__init__()
        self.df = df.reset_index(drop=True)
        self.tech_indicator_list = tech_indicator_list
        ...
        # Observation space: one float per technical indicator.
        obs_dim = len(tech_indicator_list)
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32)

        # Action space: one float in [-1,1]. We'll later scale it to a predicted price.
        ...
        self.predictions = []
        self.actuals = []
        # Track the previous predicted price; initialize it during reset.
        self.prev_predicted_price = None
    ...
    def _get_observation(self):
        """Construct the observation from the tech indicators in the current row."""
        row = self.df.loc[self.current_step]
        obs = [row[col] for col in self.tech_indicator_list]
        return np.array(obs, dtype=np.float32)
    ...
    def _get_reward(self, predicted_price: float, actual_price: float) -> float:
        """
        Compute the base reward based on prediction error.
        For 'exp' reward_type, reward = exp(-error/reward_scale).
        For 'linear', reward = -error.
        """
        error = abs(predicted_price - actual_price)
        if self.reward_type == "exp":
            #reward = np.exp(-error / self.reward_scale)
    ...
    def reset(self, *, seed=None, options=None):
        """
        Reset the environment to the fixed start index. Also initialize logs and the previous prediction.
        """
        self.terminal = False
        self.current_step = self.start_idx
        self.predictions = []
        self.actuals = []
        # For the first step, we initialize the previous predicted price with the actual close price.
        self.prev_predicted_price = float(self.df.loc[self.current_step, "close"])
        return self._get_observation(), {}

    def step(self, action):
        """
        Execute one step: scale the action using the previous predicted price as baseline,
        compute reward (including a penalty for large day-to-day changes in prediction),
        log predictions, and move to the next row.
        """

        # Use the previous day's predicted price as the baseline for scaling.
        baseline = self.prev_predicted_price if self.prev_predicted_price is not None else float(self.df.loc[self.current_step, "close"])

        # Scale the action (assumed in [-1,1]) to a predicted price.
        # For example, map [-1,1] to [0, 2*baseline]:
        alpha = (float(action[0]) + 1.0) / 2.0  # transforms to [0,1]
        #print("actionis", action[0])

        predicted_price = alpha * 1000#* baseline

        # Get actual price at current step
        actual_price = float(self.df.loc[self.current_step, "close"])

        # Log the prediction and actual price
        self.predictions.append(predicted_price)
        self.actuals.append(actual_price)

        # Compute base reward using the error
        reward = self._get_reward(predicted_price, actual_price)
        reward0 = reward
        # Add penalty if the difference between today's prediction and yesterday's prediction is large
        if baseline is not None:
            diff = 1/(abs(predicted_price - baseline) + 1e-9)
            reward = self.deviation_factor*reward + self.penalty_factor * diff

        # Update the previous predicted price for the next step
        self.prev_predicted_price = predicted_price
        ...
        # Get next observation (or a dummy observation if done)
        if done:
            obs = np.zeros(self.observation_space.shape, dtype=np.float32)
        else:
            obs = self._get_observation()
        print("vitals:", "action:", action[0], ", reward0:", reward0, "reward:", reward,
...
class PricePredictionEnvRandomStartsOld(gym.Env):
    """
    ...
    def __init__(self, df: pd.DataFrame, tech_indicator_list: list[str],
                 max_episode_length: int = 60, reward_type: str = "exp", penalty_factor: float = 0.001):
        """
        Args:
    ...
    def step(self, action):
        ...
        if self.current_step > 0:
            ...
            prev_close_price = 415
        #predicted_price = float(action[0])
        predicted_price = (float(action[0]) + 1.0)/2.0 * 415
        ...
        if self.current_step > 0:
            ...
            day_to_day_diff = abs(predicted_price - prev_close_price)
            # Example penalty: 0.01 * difference
            penalty = self.penalty_factor * day_to_day_diff
            # Subtract penalty from reward
            reward -= penalty
...
class PricePredictionEnvRandomStarts(gym.Env):
    ...
    Observations:
        - A vector of technical indicators for the current day.

    Action:
        - A single float representing the predicted price (scaled from [-1,1] to some range).

    Reward:
        - By default, uses an exponential-like reward or negative error.
        - Additional penalty can be applied if the predicted price changes too drastically
          compared to the previous day's predicted price.

    Random Starts:
        - Each episode begins at a random day in the dataset.
    """

    metadata = {"render_modes": ["human"]}

    def __init__(self, df, tech_indicator_list, max_episode_length=60, reward_type="exp",
                 penalty_factor=0.0001, deviation_factor = 0.01,  **kwargs):
        """
        Args:
            df: DataFrame with columns including 'date', 'close', and the technical indicators.
            tech_indicator_list: List of columns (features) used for prediction.
            max_episode_length: Maximum number of days (steps) per episode.
            reward_type: 'exp' for exponential reward, 'linear' for a linear reward, or fallback to negative error.
        """
        super().__init__()
        self.df = df.reset_index(drop=True)
        self.tech_indicator_list = tech_indicator_list
        ...
        # Action space: single predicted price in [-1,1], which we'll scale.
        ...
        # Track the previous day's predicted price
        self.prev_predicted_price = None

    def _seed(self, seed=None):
        """Ensure compatibility with NumPy's newer random API."""
        self.np_random, seed = seeding.np_random(seed)
        # If the returned object is not a Generator, convert it
        if not isinstance(self.np_random, np.random.Generator):
            self.np_random = np.random.default_rng(seed)
        return [seed]
    ...
    def _get_reward(self, predicted_price: float, actual_price: float) -> float:
        """
        Compute reward.
        Example: an exponential-like function or negative error.
        """
        error = abs(predicted_price - actual_price)
        if self.reward_type == "exp":
            # Simple example: larger is better, so 1/error can be used, or e^(-error)
            return 1 / error if error > 1e-9 else 10e5  # or np.exp(-error / scale)
            #return -error
        elif self.reward_type == "linear":
            # A linear style: reward ~ 1 - (error / something)
            # This can clip below 0 if error is large.
            return max(0.0, 1.0 - error / max(1.0, actual_price))
        else:
            # Default: negative absolute error
            return -error

    def reset(self, *, seed=None, options=None):
        """Reset the environment with a random start date."""
        self.terminal = False
        self.episode += 1

        # Validate max_episode_length
        if self.max_episode_length > self.num_days:
            print(
                f"⚠️ max_episode_length ({self.max_episode_length}) "
                f"exceeds number of unique days ({self.num_days}). Adjusting."
            )
            self.max_episode_length = self.num_days

        # Random start day index
        max_start = max(0, self.num_days - self.max_episode_length)
        if max_start > 0:
            self.start_day_idx = self.np_random.integers(0, max_start + 1)
        else:
            self.start_day_idx = 0
        ...
        self.predictions = []
        self.actuals = []
        self.prev_predicted_price = None  # Reset for new episode
    ...
    def step(self, action):
        """
        Execute one timestep: record the prediction, compute reward, and advance one day.
        Additionally, penalize large changes in the predicted price from the previous day.
        """
        # Use previous day's close for scaling, since that value is known when making the prediction.
        if self.current_step > 0 and self.prev_predicted_price:
            prev_predicted_price = self.prev_predicted_price #float(self.df.loc[self.current_step - 1, "close"])
        else:
            # For the very first step, if no previous data is available, you can choose a default value.
            prev_predicted_price = 415.0#float(self.df.loc[self.current_step, "close"])

        # Scale the raw action from [-1,1] to a predicted price range based on the previous day's close.
        # For example, if we want predictions to be roughly in the range [0, 2*prev_close]:
        alpha = (float(action[0]) + 1.0) / 2.0  # Convert from [-1,1] to [0,1]
        predicted_price = alpha *  1000 #prev_predicted

        # Get the actual price for reward calculation (this is the true value, revealed after prediction)
        actual_price = float(self.df.loc[self.current_step, "close"])
        ...
        # Compute base reward using your chosen reward function (e.g., exponential decay, negative error, etc.)
        reward = self._get_reward(predicted_price, actual_price)
        # Optionally, add a penalty for large changes in predicted price relative to the previous prediction.
        if prev_predicted_price is not None:
            day_to_day_diff = 1/(abs(predicted_price - prev_predicted_price) + 1e-9)
            penalty = self.penalty_factor * day_to_day_diff
            reward = self.deviation_factor * reward +  penalty

        # Update the stored previous predicted price for next step.
        self.prev_predicted_price = predicted_price

        # Advance to the next day
        self.current_step += 1

        # Check if episode is done (either end of data or reached max_episode_length)
        if self.current_step >= len(self.df) - 1:
            ...
            day_idx = self.unique_dates.index(current_date)
            day_offset = day_idx - self.start_day_idx
            done = (day_offset >= self.max_episode_length - 1)

        # Get the new observation (or return a dummy observation if done)
        if done:
            obs = np.zeros(self.observation_space.shape, dtype=np.float32)
        else:
            obs = self._get_observation()
    ...
    def get_sb_env(self):
        """Wrap in DummyVecEnv for Stable-Baselines3 usage."""
        from stable_baselines3.common.vec_env import DummyVecEnv
        env = DummyVecEnv([lambda: self])
        obs = env.reset()
        return env, obs
